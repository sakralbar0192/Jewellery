...
const DISPLAYED_SLIDE_CLASS = 'gallery__slide--shown' //класс отображаеиых слайдов
const BULLET_CLASS = 'gallery__bullet';//класс неактивных буллетов пагинации
...
const ACTIVE_BULLET_CLASS = 'gallery__bullet--active';//класс активного буллета пагинации
...
// let slidesPerView = 4; //количество единовременно отображаемых слайдов

// let pageNumber = 1; //первая показываемая страница

// determineRequiredNumberSlidesPerView(); //проверка ширины экрана


// const showSlides = () => {

//   let shownSlides = slides.slice(0, slidesPerView);
//   toggleClass(shownSlides);
// }
...
// //создание буллетов
// const createBullets = () => {
//   for (let i=1; i<=determineTheNumberOfSlidePages(); i++) {
//     const bullet = createBullet('span', BULLET_CLASS, String(i), String(i));
//     paginationContainer.appendChild(bullet);
//   }
// }
...
// const bullets = Array.from(paginationContainer.children);
// bullets[pageNumber-1].classList.toggle(ACTIVE_BULLET_CLASS);

// //обработчик событий на буллетах
// bullets.forEach(bullet => {
//   bullet.addEventListener('click', (e) => {
//     bullets[pageNumber-1].classList.toggle(ACTIVE_BULLET_CLASS);
//     pageNumber = parseInt(e.target.getAttribute('data-bullet-number'));
//     bullets[pageNumber-1].classList.toggle(ACTIVE_BULLET_CLASS);
//     toggleClass(shownSlides);
//     shownSlides = slides.slice(slidesPerView*(pageNumber-1),slidesPerView*(pageNumber-1)+slidesPerView);
//     toggleClass(shownSlides);
//   })
// })

// //настройка работы кнопок навигации
// //слайд вперед
// buttonNext.addEventListener('click', () => {
//   bullets[pageNumber-1].classList.toggle(ACTIVE_BULLET_CLASS);
//   pageNumber++;
//   if (pageNumber > determineTheNumberOfSlidePages()) {
//     pageNumber = determineTheNumberOfSlidePages()
//   }
//   bullets[pageNumber-1].classList.toggle(ACTIVE_BULLET_CLASS);
//   toggleClass(shownSlides);
//   shownSlides = slides.slice(slidesPerView*(pageNumber-1),slidesPerView*(pageNumber-1)+slidesPerView);
...
// window.addEventListener('resize', () => {
//   const requiredSlides = determineRequiredNumberSlidesPerView();
//   if (requiredSlides != slidesPerView) {
