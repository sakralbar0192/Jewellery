...
*/
{
  ...
  const MOBILE_BREAK_POINT = 768;//максимальная ширина смартфонов
  const HIDDEN_SLIDE_CLASS = 'slider__slide--hidden' //класс скрытых слайдов
  ...
  const slides = Array.from(slider.children);//массив слайдов в слайдере
  ...
  const createPagination = (numberOfSlidePages, paginationContainer, currentPage, activePageClass, typeOfPagination) => {
    ...
    if (typeOfPagination === 'mobile') {
      const element = document.createElement('span');
      element.textContent = 'of';
      paginationContainer.appendChild(element);
      const numberPagesElement = document.createElement('span');
      numberPagesElement.textContent = numberOfSlidePages;
      paginationContainer.appendChild(numberPagesElement);
    }
  }

  //проверка ширины экрана
  const determineRequiredNumberSlidesPerView = (slidesPerViewDesktop, breakpoint, slidesPerViewTablet) => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    slides.forEach(slide => {
      toggleClass(slide, HIDDEN_SLIDE_CLASS);
    })
    ...
    //выведем на экран слайды с активной страницы
    displayedSlides.forEach(element => {
      toggleClass(element, DISPLAYED_SLIDE_CLASS);
      toggleClass(element, HIDDEN_SLIDE_CLASS);
    })

    //заведем переменную с типом пагинации и примем ее равной 'tablet'
    let typeOfPagination = 'tablet';

    //.определим вид пагинации для рендера
    if (window.innerWidth < MOBILE_BREAK_POINT) {
      typeOfPagination = 'mobile'
      createPagination(numberOfSlidePages, paginationContainer, activePage, ACTIVE_BULLET_CLASS, typeOfPagination)
    } else {
      createPagination(numberOfSlidePages, paginationContainer, activePage, ACTIVE_BULLET_CLASS, typeOfPagination)
    }
    ...
    window.addEventListener('resize', () => {
      //определим, какой тип пагинации необходимо отобразить
      let requiredTypeOfPagination = '';
      if (window.innerWidth < MOBILE_BREAK_POINT) {
        requiredTypeOfPagination = 'mobile';
      }else {
        requiredTypeOfPagination = 'tablet';
      }
      //проверим нужно ли перерендерить пагинацию
      if (typeOfPagination != requiredTypeOfPagination) {
        typeOfPagination = requiredTypeOfPagination;
        paginationContainer.innerHTML = '';
        createPagination(numberOfSlidePages, paginationContainer, activePage, ACTIVE_BULLET_CLASS, typeOfPagination);
        //обновим буллеты в массиве
        bullets = Array.from(paginationContainer.children);
        //настроим работу буллетов, если тип пагинации не 'mobile'
        if (typeOfPagination != 'mobile') {
          //настроим работу буллетов пагинации
          bullets.forEach(bullet => {
            bullet.addEventListener('click', (e) => {
    ...
    //обработаем поведение слайдера при свайпе
    //заведем необходимые переменные
    const DIST_THRESHOLD = 150//минимальное растяние, необходимое для того чтобы свайп был засчитан
    let startX;//начальная х-координата свайпа
    let dist; //длина свайпа

    //заведем обработчик события начала свайпа по слайдеру
    slider.addEventListener('touchstart', (e) => {
      let touchobj = e.changedTouches[0];
      dist = 0;//обнуляем длину свайпа
    ...
    slider.addEventListener('touchend', (e) => {
      let touchobj = e.changedTouches[0];
      dist = touchobj.pageX-startX;//считаем длину свайпа
      //проверим что длина свайпа достаточна чтобы считать это жестом переключения
      if (Math.abs(dist) >= DIST_THRESHOLD) {
        //определим в какую сторону был свайп
        if (dist > 0) {
          scrollSlides(false);
        } else {
          scrollSlides(true);
